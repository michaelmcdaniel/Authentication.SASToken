...
namespace Authentication.SASToken.Authentication
{
	public class SASTokenAuthenticationHandler<T> : AuthenticationHandler<T> where T: SASTokenAuthenticationOptions, new()
	{
		...
		protected override async Task<AuthenticateResult> HandleAuthenticateAsync()
		{
			...
			var tokenSource = await store.GetAsync(token.Value);
			...
			if (!tokenSource.Value.Validate(token.Value, Request, base.Logger))
			{
				return AuthenticateResult.Fail("Invalid SASToken");
			}
			var claims = new[] {
					new Claim(System.Security.Claims.ClaimTypes.NameIdentifier, token.Value.Id.ToString()),
